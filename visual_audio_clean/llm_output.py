...
def create_llm_input(results_data, transcript=None, output_path="llm_analysis_input.json", fps=30):
    ...
    Args:
        ...
        transcript: Full transcript of the video (string)
    ...
    llm_data = {
        ...
        "fps": fps,
        "transcript": transcript,  # Changed from "sentences in chunk" to "transcript"
...
def create_llm_input_with_bbox(results_data, transcript=None, output_path="llm_analysis_input_with_bbox.json", fps=30):
    """
    Create a modified version of the LLM input JSON that includes bounding boxes.
    
    Args:
        results_data: The main analysis result dictionary.
        transcript: Transcript data.
        output_path: Path to save the JSON file.
        fps: Frames per second of the video.
        
    Returns:
        The structured data dictionary with bounding boxes included.
    """
    # Start with a similar structure as the regular LLM input data
    llm_data = {
        "session_summary": {
            "total_frames_processed": len([k for k in results_data.keys() if k not in ['profiles', 'visualizations']]),
            "distinct_people_identified": {}  # Summary based on FINAL profiles
    ...
    # Populate frame-by-frame analysis WITH bounding boxes
    for frame_path, frame_data in results_data.items():
        if frame_path in ['profiles', 'visualizations']:
            continue  # Skip metadata keys
        ...
        for face_info in frame_data.get('faces', []):
            ...
            people_with_boxes.append({
                ...
                "emotion_detected": emotion_data,
                "gaze_info": face_info.get('gaze', {}),
                "bbox": face_info['bbox_pixels'],  # Keep bbox for this version
                "profile_id": final_profile_id
            })

        # Second pass: create final output with gaze targets but KEEP bounding boxes
        for person in people_with_boxes:
            # Prepare person data WITH bounding box
            person_data = {
                "name": person["name"],
                "emotion_detected": person["emotion_detected"],
                "bbox": person["bbox"],  # Keep the bounding box in this version
                "gaze_info": None
            }

            # Add gaze target information if available (same logic as original)
            gaze_info = person.get("gaze_info", {})
            if gaze_info:
                gaze_data = {}
    ...
    try:
        with open(output_path, 'w') as f:
            json.dump(llm_data, f, indent=2, cls=NumpyEncoder)
        print(f"LLM analysis input with bounding boxes saved successfully to {output_path}")
    except Exception as e:
        print(f"Error saving LLM JSON with bounding boxes to {output_path}: {e}")
